...
// FluidNCEspNowClient local fix: remembered status state moved to file scope so it can
// be cleared on machine change via fnc_parser_reset().
static bool  last_probe_pin   = false;
...
static void parse_status_report(char* field) {
    ...
    new_control_pins[0] = '\0';  // FluidNCEspNowClient local fix: reset once per report, not per field
    ...
    while (*next) {
        ...
        if (strcmp(field, "Ln") == 0) {
            // n
            linenum     = atoi(value);
            has_linenum = true;  // FluidNCEspNowClient local fix: upstream never sets this
            continue;
        }
    ...
    if (has_fs) {  // FluidNCEspNowClient local fix: do not report uninitialized feed/spindle
        show_feed_spindle(fs[0], fs[1]);
    }
    if (has_override) {
        show_overrides(frs[0], frs[1], frs[2]);
    }
    if (new_probe != last_probe_pin) {  // FluidNCEspNowClient local fix: remembered state must persist to clear
        last_probe_pin = new_probe;
        show_probe_pin(new_probe);
    }
...
static void parse_probe(char* body) {
    ...
    if (!split(body, &next, ':')) {  // FluidNCEspNowClient local fix: PRB uses ':' not '|'
        ...
        if (strlen(tag) > 1) {
            switch (*tag) {
                case 'T':
                    ...
                    break;
                case 'F':
                    new_gcode_modes.feed = atoi(tag + 1);  // FluidNCEspNowClient local fix: upstream left this unparsed
                    break;
                case 'S':
...
void collect(uint8_t data) {
    ...
    if (_report_len < REPORT_BUFFER_LEN - 1) {  // FluidNCEspNowClient local fix: bound the buffer
...
// FluidNCEspNowClient local addition: clear remembered parser state, e.g. when
// switching to a different machine.
void fnc_parser_reset() {
    _report[0]          = '\0';
    _report_len         = 0;
    _ackwait            = false;
    control_pins[0]     = '\0';
    new_control_pins[0] = '\0';
    last_probe_pin      = false;
    memset(wcos, 0, sizeof(wcos));
    memset(&old_gcode_modes, 0, sizeof(old_gcode_modes));
    memset(&new_gcode_modes, 0, sizeof(new_gcode_modes));
}
